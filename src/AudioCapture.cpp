...
AudioCapture::AudioCapture(const Config &config)
  ...
  try {
    ...
    std::cout << "RtAudio initialized with " << rtAudio_->getDeviceCount()
              << " devices" << std::endl;
  ...
  if (config_.deviceId == 0 || config_.deviceId >= rtAudio_->getDeviceCount()) {
    ...
    for (unsigned int i = 0; i < deviceCount; i++) {
      ...
      if (info.inputChannels > 0) {
        ...
        std::cout << "Using audio input device: " << info.name << " (ID: " << i
                  << ")" << std::endl;
        break;
      }
    }
    ...
    if (info.inputChannels == 0) {
      std::cerr << "Device ID " << config_.deviceId << " has no input channels!"
                << std::endl;
      return false;
    }
    std::cout << "Using specified audio input device: " << info.name
              << " (ID: " << config_.deviceId << ")" << std::endl;
  }

  // Create audio buffer
  audioBuffer_ =
      std::make_unique<AudioBuffer>(config_.sampleRate * 2); // 2 seconds buffer
...
int AudioCapture::portAudioCallback(const void *inputBuffer, void *outputBuffer,
  ...
  if (capture && inputBuffer && capture->isCapturing_.load()) {
    capture->processAudioData(inputBuffer, framesPerBuffer,
...
bool AudioCapture::startRtAudio() {
  ...
  if (config_.channels > deviceInfo.inputChannels) {
    std::cout << "Reducing channels from " << config_.channels << " to "
              << deviceInfo.inputChannels << std::endl;
  ...
  try {
    rtAudio_->openStream(nullptr,             // No output
...
bool AudioCapture::isCapturing() const { return isCapturing_.load(); }
...
void AudioCapture::processAudioData(const void *inputBuffer,
                                    unsigned int frames, double timestamp) {
  ...
  std::vector<float> floatData =
      convertToFloat(inputBuffer, frames, RTAUDIO_FLOAT32);
...
std::vector<float> AudioCapture::convertToFloat(const void *input,
                                                unsigned int frames,
                                                int format) const {
