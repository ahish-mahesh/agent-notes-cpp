...
llama_bridge_context *llama_bridge_init(llama_bridge_params params) {
  ...
  bridge_ctx->model =
      llama_model_load_from_file(params.model_path, model_params);
  if (!bridge_ctx->model) {
    llama_backend_free();
    delete bridge_ctx;
    return nullptr;
  ...
  if (params.top_p > 0.0f && params.top_p <= 1.0f) {
    llama_sampler_chain_add(bridge_ctx->sampler,
...
llama_bridge_result llama_bridge_generate(llama_bridge_context *ctx,
  ...
  for (int i = 0; i < max_tokens; ++i) {
    ...
    char token_str[256];
    int n = llama_token_to_piece(vocab, next_token, token_str,
                                 sizeof(token_str), 0, false);
    if (n < 0) {
      result.success = false;
...
llama_bridge_result llama_bridge_chat(llama_bridge_context *ctx,
...
llama_bridge_tokens llama_bridge_tokenize(llama_bridge_context *ctx,
                                          const char *text) {
  ...
  const struct llama_vocab *vocab = llama_model_get_vocab(ctx->model);
  int n_tokens = llama_tokenize(vocab, text, strlen(text), tokens.data(),
                                tokens.size(), true, false);
...
char *llama_bridge_detokenize(llama_bridge_context *ctx,
                              const llama_bridge_tokens *tokens) {
  ...
  for (int i = 0; i < tokens->count; i++) {
    char token_str[256];
    const struct llama_vocab *vocab = llama_model_get_vocab(ctx->model);
    int n = llama_token_to_piece(vocab, tokens->tokens[i], token_str,
                                 sizeof(token_str), 0, false);
    if (n > 0) {
