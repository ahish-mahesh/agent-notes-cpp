...
struct whisper_bridge_context {
  ...
  whisper_bridge_context()
      : ctx(nullptr), callback(nullptr), user_data(nullptr), streaming(false) {}
};
...
whisper_bridge_context *whisper_bridge_init(whisper_bridge_params params) {
  ...
  bridge_ctx->ctx =
      whisper_init_from_file_with_params(params.model_path, cparams);
  if (!bridge_ctx->ctx) {
    delete bridge_ctx;
    return nullptr;
  }
...
whisper_bridge_transcribe_audio(whisper_bridge_context *ctx,
                                const float *audio_data, int audio_len) {
  ...
  result.confidence =
      0.9f; // Placeholder - whisper doesn't provide confidence scores
  result.start_time_ms =
      n_segments > 0 ? whisper_full_get_segment_t0(ctx->ctx, 0) * 10 : 0;
  result.end_time_ms =
      n_segments > 0
          ? whisper_full_get_segment_t1(ctx->ctx, n_segments - 1) * 10
          : 0;
...
bool whisper_bridge_start_stream(whisper_bridge_context *ctx,
                                 whisper_bridge_callback callback,
                                 void *user_data) {
...
void whisper_bridge_add_audio(whisper_bridge_context *ctx,
                              const float *audio_data, int audio_len) {

  if (!ctx || !ctx->streaming || !audio_data)
    return;
