...
WhisperTranscriber::WhisperTranscriber(const Config &config)
    : config_(config), whisperContext_(nullptr), initialized_(false),
      shouldStop_(false), bufferStartTime_(0.0) {
...
std::vector<WhisperTranscriber::Result>
WhisperTranscriber::transcribe(const std::vector<float> &audioData) {
...
void WhisperTranscriber::addAudioData(const std::vector<float> &audioData,
                                      double timestamp) {
  if (!initialized_ || audioData.empty()) {
    return;
  }
...
void WhisperTranscriber::startRealTimeProcessing(
  ...
  processingThread_ =
      std::thread(&WhisperTranscriber::processingThreadFunction, this);
...
bool WhisperTranscriber::isInitialized() const { return initialized_; }

std::vector<std::string> WhisperTranscriber::getSupportedLanguages() {
  return {"auto", "en", "zh", "de", "es",  "ru", "ko", "fr", "ja", "pt",
...
void WhisperTranscriber::processingThreadFunction() {
  ...
  while (!shouldStop_.load()) {
    ...
    while (!audioQueue_.empty() && !shouldStop_.load()) {
      ...
      // Add to buffer
      audioBuffer_.insert(audioBuffer_.end(), audioData.first.begin(),
                          audioData.first.end());
...
bool WhisperTranscriber::detectSpeech(
...
std::vector<WhisperTranscriber::Result> WhisperTranscriber::extractResults(
    const whisper_bridge_result &bridge_result) const {
  std::vector<Result> results;

  if (bridge_result.text && strlen(bridge_result.text) > 0) {
    Result result;
    result.text = bridge_result.text;
    result.startTime = bridge_result.start_time_ms /
    ...
    result.text.erase(
        std::find_if(result.text.rbegin(), result.text.rend(),
                     [](unsigned char ch) { return !std::isspace(ch); })
            .base(),
        result.text.end());
...
void WhisperTranscriber::printSystemInfo() const {
  ...
  std::cout << "  Language: "
            << (config_.language == "auto" ? "auto-detect" : config_.language)
            << std::endl;
